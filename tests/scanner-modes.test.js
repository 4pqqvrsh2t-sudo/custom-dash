...
test('scanner modes keep measured detections opt-in and use five metre display steps',()=>{
  ...
  const world=q('#scanner-world'),all=(root,predicate)=>root.children.flatMap(e=>[...(predicate(e)?[e]:[]),...all(e,predicate)]);assert.equal(all(world,e=>e.attrs.class==='contact-lock').length,0);q('#scan-mode').onchange({target:{value:'map'}});assert.equal(all(world,e=>e.attrs.class==='map-building').length,0);q('#perception-preview').onchange({target:{checked:true}});assert.equal(all(world,e=>e.attrs.class==='map-building').length,9);assert(all(world,e=>e.attrs.class==='map-destination').length);
  q('#perception-preview').onchange({target:{checked:false}});q('#scan-mode').onchange({target:{value:'vehicle'}});assert.equal(all(world,e=>e.attrs.class==='contact-lock').length,0);
...
test('centered coverage bands, directional symbols, and selectable confidence use supplied measurements',()=>{
 ...
 const bands=all(q('#scanner-world'),e=>e.attrs.class==='confidence-zone');assert.equal(bands.length,1);assert.equal(bands[0].attrs.cx,180);assert.equal(bands[0].attrs.cy,180);
