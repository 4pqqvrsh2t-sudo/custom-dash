...
test('scanner modes keep measured detections opt-in and use five metre display steps',()=>{
  ...
  q('#scan-vehicles').onchange({target:{checked:true}});assert.equal(world.children.length,3);
...
test('sensor bridge validates measurements and exposes confidence hue',()=>{
  ...
  assert.equal(q('#scanner-world').children.filter(e=>e.attrs.class==='confidence-area').length,1);
