...
test('centered coverage bands, directional symbols, and selectable confidence use supplied measurements',()=>{
 ...
 const input=['CAR','CAR','HUMAN','ANIMAL','MISC'].map((type,i)=>({id:String(i),type,distanceMeters:20+i*10,bearingDegrees:i*30,confidence:.9,...(i===0?{headingDegrees:90}:{})}));
 c.SurfacePerception.ingestContacts(input,[{radiusMeters:50,confidence:.8}]);const marks=q('#scanner-world').children.filter(e=>e.attrs.class==='contact-lock');assert.deepEqual(marks.map(e=>e.tag),['path','rect','path','rect','circle']);assert.match(marks[0].attrs.transform,/rotate\(90/);assert.equal(marks[4].attrs['stroke-dasharray'],'1 3');marks[2].onclick();assert.match(q('#contact-detail').textContent,/90%/);
 const bands=q('#scanner-world').children.filter(e=>e.attrs.class==='confidence-zone');assert.equal(bands.length,1);assert.equal(bands[0].attrs.cx,180);assert.equal(bands[0].attrs.cy,110);
 c.SurfacePerception.test([]);assert.equal(c.SurfacePerception.state().contactCount,5);c.SurfacePerception.test(null);assert.equal(q('#scanner-world').children.filter(e=>e.attrs.class==='contact-lock').length,5);
});
