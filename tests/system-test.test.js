...
test('system test walks all systems, restores previews, and stops on measured motion',async()=>{
 ...
 assert(frames.some(x=>x?.length===200));assert(frames.some(x=>x?.length===25));assert(readings.some(x=>x.speed===150&&x.rpm===7500));assert.equal(readings.at(-1).speed,0);assert.equal(frames.at(-1),null);assert.equal(views.at(-1),'systems');assert.deepEqual(tones.slice(-3),['info','warning','critical']);assert.equal(start.disabled,false);assert.equal(q('#pin-preview').checked,false);
