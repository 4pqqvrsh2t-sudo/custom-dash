...
(() => {
  ...
  function normalizeContact(item,index){
    ...
    const distance=Number(item.distanceMeters),bearing=Number(item.bearingDegrees),confidence=item.confidence==null?null:Number(item.confidence);
    ...
    return {id:String(item.id||index),type,distanceMeters:distance,bearingDegrees:bearing,confidence:confidence===null?null:clamp(confidence,0,1),sensor:String(item.sensor||'UNSPECIFIED').toUpperCase().slice(0,20)};
  }
  ...
  function draw(parent,kind,full=false){
    ...
    if(kind==='map'||kind==='road'){
      ...
      element('path',{d:'M180 110L123 31A164 85 0 0 1 237 31Z',fill:'#78dce710',stroke:'#78dce733',class:'sensor-fov wide'},parent);
      element('path',{d:'M180 110L165 26A164 85 0 0 1 195 26Z',fill:'#ffab540e',stroke:'#ffab5438',class:'sensor-fov narrow'},parent);
  ...
  function render(){
    ...
    const hasLive=liveContacts.length>0;q('#scanner-source').textContent=mode==='map'||mode==='road'?'SCHEMATIC MAP':preview?'SIMULATED CONTACTS':hasLive?'LIVE SENSOR INPUT':'NO SENSOR DATA';
  ...
  globalThis.SurfacePerception={ingestContacts(items){liveContacts=Array.isArray(items)?items.map(normalizeContact).filter(Boolean):[];preview=false;if(q('#perception-preview'))q('#perception-preview').checked=false;render();return liveContacts.length;},clear(){liveContacts=[];render();},state(){return {rangeMeters,mode,preview,contactCount:liveContacts.length};},confidenceColor};
  render();setInterval(()=>{if(document.hidden||!preview||q('#animations').checked===false||matchMedia('(prefers-reduced-motion: reduce)').matches)return;if(!q('#cockpit').classList.contains('active')&&!section.classList.contains('active'))return;phase+=.12;draw(q('#scanner-world'),mode);draw(q('#proximity-scope'),'proximity',true);renderContacts();},250);if(location.hash==='#proximity')showView('proximity');
})();
