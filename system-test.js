...
(()=>{
 ...
 function sweep(view){showView(view);animate(8000,p=>{const wave=p<=.5?p*2:(1-p)*2;window.SurfaceDisplayTest.frame({speed:150*wave,rpm:7500*wave,coolant:160+55*wave,voltage:12+2.5*wave,load:wave*100,throttle:wave*100});const limit=q('#speed-limit');limit.textContent=String([15,25,35,45,55,65,70][Math.min(6,Math.floor(p*7))]);q('.speed-limit-card small').textContent='TEST LIMIT / SIMULATED';});}
 const steps=[
 {view:'proximity',label:'MULTIPLE TARGETS',duration:8000,run(){animate(7800,p=>{SurfacePerception.test(Array.from({length:25},(_,i)=>({...targets[i%5],id:'moving-'+i,distanceMeters:20+(i*17+p*80)%170,bearingDegrees:i*137.5+p*40,headingDegrees:i%5===0?(i*45+p*90)%360:undefined})));});window.radarTone?.('arrival',true);}},
 {view:'proximity',label:'TARGETS DEPARTING',duration:3000,run(){SurfacePerception.test(targets);window.radarTone?.('departure',true);}},
 {view:'proximity',label:'200 TARGET LOAD',duration:3500,run(){SurfacePerception.test(Array.from({length:200},(_,i)=>({...targets[i%5],id:'load-'+i,distanceMeters:10+(i*13)%185,bearingDegrees:i*137.5})));}},
 {view:'data',label:'SPEED / RPM / LIVE CHART SWEEP',duration:8200,run(){sweep('data');}},
 {view:'navigation',label:'NAVIGATION SPEED SWEEP',duration:8200,run(){sweep('navigation');}},
 {view:'cockpit',label:'SPEED LIMIT DISPLAY',duration:8200,run(){sweep('cockpit');}},
 ...
 function next(){if(index>=steps.length){restore();q('#test-step').textContent='COMPLETE';q('#stop-system-test').textContent='CLOSE';return;}const step=steps[index++];showView(step.view);q('#test-step').textContent=index+' / '+steps.length+' — '+step.label;step.run();timer=setTimeout(next,step.duration||1800);}
