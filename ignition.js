...
(() => {
  ...
  function stopHum(){if(hum){try{hum.stop();hum.disconnect();humGain.disconnect()}catch{}hum=null;humGain=null}}
  function startHum(c){if(closed||document.hidden||c.state!=='running'||hum)return;hum=c.createOscillator();humGain=c.createGain();hum.type='sine';hum.frequency.setValueAtTime(42,c.currentTime);hum.frequency.linearRampToValueAtTime(67,c.currentTime+8);humGain.gain.setValueAtTime(.035*prefs.volume/100,c.currentTime);hum.connect(humGain).connect(c.destination);hum.start();}
  ...
  function finish(){closed=true;clearTimeout(timer);stopHum();overlay.hidden=true;shell.inert=false;document.body.classList.remove('booting');document.dispatchEvent(new Event('surface-startup-complete'));q('nav button[aria-current]')?.focus({preventScroll:true});}
