...
function renderData(){const snap=frozenSnapshot||telemetry.snapshot(),rows=snap.history,win=Math.max(.001,snap.elapsed-(rows[0]?.t??snap.elapsed));$('#session-time').textContent=snap.count?formatTime(snap.elapsed):'—';$('#session-distance').textContent=snap.speedCount===0?'—':snap.distance.toFixed(2);$('#session-average').textContent=snap.average===null?'—':snap.average.toFixed(1);$('#session-peak').textContent=snap.peak===null?'—':snap.peak.toLocaleString();$('#sample-count').textContent=snap.count?snap.count+' REAL SAMPLES':'NO SENSOR SAMPLES';$('#history-status').textContent=frozenSnapshot?'VIEW HELD':'LIVE / EVERY RECEIVED SAMPLE';
  for(const m of metrics){const measured=rows.filter(r=>Number.isFinite(r[m.key])),vals=measured.map(r=>r[m.key]),last=vals.at(-1);$('#value-'+m.key).textContent=last===undefined||(!frozenSnapshot&&!Number.isFinite(liveTelemetry[m.key]))?'—':last.toFixed(m.digits);for(const k of ['min','avg','max'])$('#'+k+'-'+m.key).textContent=vals.length?(k==='min'?Math.min(...vals):k==='max'?Math.max(...vals):vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(m.digits):'—';
