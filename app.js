...
function stopAmbient(){ambient.forEach(o=>{try{o.stop();o.disconnect();}catch{}});ambient=[];if(gain){gain.disconnect();gain=null;}}
...
function scanNoise(){try{const c=audioContext(),buffer=c.createBuffer(1,c.sampleRate*.25,c.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<data.length;i++)data[i]=(Math.random()*2-1)*.2;const node=c.createBufferSource();node.buffer=buffer;node.loop=true;gain=c.createGain();gain.gain.value=prefs.volume/100*.07;node.connect(gain).connect(c.destination);node.start();ambient.push(node);}catch{}}
...
const telemetry=new TelemetrySession();let frozenSnapshot=null;
const metrics=[{key:'speed',label:'GROUND SPEED',unit:'MPH',min:0,max:80,digits:0},{key:'rpm',label:'ENGINE SPEED',unit:'RPM',min:0,max:4000,digits:0},{key:'coolant',label:'COOLANT TEMP',unit:'°F',min:160,max:220,digits:1},{key:'voltage',label:'SYSTEM VOLTAGE',unit:'V',min:12,max:15,digits:2}];
...
function renderData(){const snap=frozenSnapshot||telemetry.snapshot(),win=Number($('#history-window').value),rows=snap.history.filter(r=>r.t>snap.elapsed-win);$('#session-time').textContent=snap.count?formatTime(snap.elapsed):'—';$('#session-distance').textContent=snap.speedCount===0?'—':snap.distance.toFixed(2);$('#session-average').textContent=snap.average===null?'—':snap.average.toFixed(1);$('#session-peak').textContent=snap.peak===null?'—':snap.peak.toLocaleString();$('#sample-count').textContent=snap.count?snap.count+' REAL SAMPLES':'NO SENSOR SAMPLES';$('#history-status').textContent='LAST '+win+' SECONDS';
  for(const m of metrics){const measured=rows.filter(r=>Number.isFinite(r[m.key])),vals=measured.map(r=>r[m.key]),last=vals.at(-1);$('#value-'+m.key).textContent=last===undefined?'—':last.toFixed(m.digits);for(const k of ['min','avg','max'])$('#'+k+'-'+m.key).textContent=vals.length?(k==='min'?Math.min(...vals):k==='max'?Math.max(...vals):vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(m.digits):'—';
    const points=measured.map(r=>[35+315*(1-(snap.elapsed-r.t)/win),124-112*Math.max(0,Math.min(1,(r[m.key]-m.min)/(m.max-m.min)))]),chart=$('#chart-'+m.key),path=points.map((p,i)=>(i?'L':'M')+p.map(v=>v.toFixed(2)).join(' ')).join(' ');chart.querySelector('.trace').setAttribute('d',path);chart.querySelector('.area').setAttribute('d',points.length?path+' L'+points.at(-1)[0]+' 124 L'+points[0][0]+' 124 Z':'');const dot=chart.querySelector('.point');dot.style.display=points.length?'':'none';if(points.length){dot.setAttribute('cx',points.at(-1)[0]);dot.setAttribute('cy',points.at(-1)[1]);}chart.querySelector('.start-label').textContent='−'+win+'s';chart.setAttribute('aria-label',m.label+': '+(last===undefined?'data unavailable':last.toFixed(m.digits)+' '+m.unit)+', '+measured.length+' measured samples');}
}
...
function updateTelemetry(){SpeedGauge.render(liveTelemetry.speed);$('#speed').textContent=formatReading(liveTelemetry.speed);$('#rpm').textContent=Number.isFinite(liveTelemetry.rpm)?Math.round(liveTelemetry.rpm).toLocaleString():'—';$$('.live-speed').forEach(e=>e.textContent=formatReading(liveTelemetry.speed));$$('.live-rpm').forEach(e=>e.textContent=Number.isFinite(liveTelemetry.rpm)?Math.round(liveTelemetry.rpm).toLocaleString():'—');$$('.live-coolant').forEach(e=>e.textContent=formatReading(liveTelemetry.coolant));$$('.live-voltage').forEach(e=>e.textContent=formatReading(liveTelemetry.voltage,1));$$('.live-fuel').forEach(e=>e.textContent=formatReading(liveTelemetry.fuel));$$('.live-load').forEach(e=>e.textContent=formatReading(liveTelemetry.load));$$('.live-throttle').forEach(e=>e.textContent=formatReading(liveTelemetry.throttle));$$('.live-intake').forEach(e=>e.textContent=formatReading(liveTelemetry.intake));$('#telemetry-source').textContent=telemetrySource==='NONE'?'NO LIVE SOURCE':telemetrySource+' / LIVE';if(!frozenSnapshot)$('#capture-status').textContent=telemetrySource==='NONE'?'NO LIVE SOURCE':telemetrySource+' / LIVE';if($('#data').classList.contains('active'))renderData();window.dispatchEvent(new CustomEvent('surface-telemetry',{detail:{...liveTelemetry,source:telemetrySource}}));}
function ingestTelemetry(values,source='EXTERNAL'){
  ...
  if(!Object.keys(clean).length)return false;const now=Date.now(),dt=lastTelemetryAt?Math.min(5,Math.max(.1,(now-lastTelemetryAt)/1000)):1;lastTelemetryAt=now;Object.assign(liveTelemetry,clean);for(const key of Object.keys(clean))telemetrySeen[key]=now;telemetrySource=String(source).slice(0,24).toUpperCase();telemetry.record(clean,dt);updateTelemetry();return true;
}
window.SurfaceTelemetry={ingest:ingestTelemetry,current:()=>({...liveTelemetry,source:telemetrySource})};
function clearStaleTelemetry(forceSpeed=false){const now=Date.now();let changed=false;for(const key of Object.keys(liveTelemetry))if(Number.isFinite(liveTelemetry[key])&&(forceSpeed&&key==='speed'||now-(telemetrySeen[key]||0)>10000)){liveTelemetry[key]=null;changed=true}if(changed){if(!Object.values(liveTelemetry).some(Number.isFinite))telemetrySource='NONE';updateTelemetry()}}
...
$('#history-window').onchange=renderData;
$('#freeze-data').onclick=()=>{frozenSnapshot=frozenSnapshot?null:telemetry.snapshot();$('#freeze-data').textContent=frozenSnapshot?'RESUME GRAPHS':'FREEZE GRAPHS';$('#capture-status').textContent=frozenSnapshot?'Ⅱ FROZEN VIEW':telemetrySource==='NONE'?'NO LIVE SOURCE':telemetrySource+' / LIVE';$('#capture-status').classList.toggle('frozen',!!frozenSnapshot);renderData();};
$('#reset-data').onclick=()=>{telemetry.reset();frozenSnapshot=null;$('#freeze-data').textContent='FREEZE GRAPHS';$('#capture-status').textContent=telemetrySource==='NONE'?'NO LIVE SOURCE':telemetrySource+' / LIVE';$('#capture-status').classList.remove('frozen');renderData();toast('Telemetry session reset');};
