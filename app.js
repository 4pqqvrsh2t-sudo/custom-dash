...
$('#chart-grid').innerHTML=metrics.map(m=>`<article class="panel chart-panel"><div class="chart-top"><h2>${m.label}</h2><div class="chart-reading"><span id="value-${m.key}">—</span><small>${m.unit}</small></div></div><svg class="chart" id="chart-${m.key}" viewBox="0 0 360 150" role="img" aria-label="${m.label} graph in ${m.unit}, simulated data"><g>${[0,.5,1].map(f=>`<path class="grid-line" d="M35 ${12+112*f}H350"/><text x="0" y="${15+112*f}">${m.max-(m.max-m.min)*f}</text>`).join('')}</g><path class="area"/><path class="trace"/><circle class="point" r="2.5" cx="350" cy="124"/><text x="35" y="147" class="start-label">−60s</text><text x="327" y="147">NOW</text></svg><div class="chart-stats"><span>MIN<b id="min-${m.key}">—</b></span><span>AVG<b id="avg-${m.key}">—</b></span><span>MAX<b id="max-${m.key}">—</b></span></div></article>`).join('');
function renderData(){const snap=frozenSnapshot||telemetry.snapshot(),win=Number($('#history-window').value),rows=snap.history.filter(r=>r.t>snap.elapsed-win);$('#session-time').textContent=formatTime(snap.elapsed);$('#session-distance').textContent=snap.distance.toFixed(2);$('#session-average').textContent=snap.average.toFixed(1);$('#session-peak').textContent=snap.peak.toLocaleString();$('#sample-count').textContent=snap.count+' SAMPLES';$('#history-status').textContent='LAST '+win+' SECONDS';
  for(const m of metrics){const vals=rows.map(r=>r[m.key]),last=vals.at(-1);$('#value-'+m.key).textContent=last===undefined?'—':last.toFixed(m.digits);for(const k of ['min','avg','max'])$('#'+k+'-'+m.key).textContent=vals.length?(k==='min'?Math.min(...vals):k==='max'?Math.max(...vals):vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(m.digits):'—';
    const points=rows.map(r=>[35+315*(1-(snap.elapsed-r.t)/win),124-112*Math.max(0,Math.min(1,(r[m.key]-m.min)/(m.max-m.min)))]),chart=$('#chart-'+m.key),path=points.map((p,i)=>(i?'L':'M')+p.map(v=>v.toFixed(2)).join(' ')).join(' ');chart.querySelector('.trace').setAttribute('d',path);chart.querySelector('.area').setAttribute('d',points.length?path+' L'+points.at(-1)[0]+' 124 L'+points[0][0]+' 124 Z':'');const dot=chart.querySelector('.point');dot.style.display=points.length?'':'none';if(points.length){dot.setAttribute('cx',points.at(-1)[0]);dot.setAttribute('cy',points.at(-1)[1]);}chart.querySelector('.start-label').textContent='−'+win+'s';chart.setAttribute('aria-label',m.label+': '+(last===undefined?'no samples':last.toFixed(m.digits)+' '+m.unit)+', '+rows.length+' simulated samples in the last '+win+' seconds');}
}
function updateTelemetry(row){SpeedGauge.render(row.speed);$('#speed').textContent=row.speed;$('#rpm').textContent=row.rpm.toLocaleString();$$('.live-speed').forEach(e=>e.textContent=row.speed);$$('.live-rpm').forEach(e=>e.textContent=row.rpm.toLocaleString());$$('.live-coolant').forEach(e=>e.textContent=row.coolant.toFixed(0));$$('.live-voltage').forEach(e=>e.textContent=row.voltage.toFixed(1));$$('.live-fuel').forEach(e=>e.textContent=row.fuel.toFixed(0));$$('.live-load').forEach(e=>e.textContent=row.load.toFixed(0));$$('.live-throttle').forEach(e=>e.textContent=row.throttle.toFixed(0));$$('.live-intake').forEach(e=>e.textContent=row.intake.toFixed(0));if($('#data').classList.contains('active'))renderData();}
$('#history-window').onchange=renderData;
$('#freeze-data').onclick=()=>{frozenSnapshot=frozenSnapshot?null:telemetry.snapshot();$('#freeze-data').textContent=frozenSnapshot?'RESUME GRAPHS':'FREEZE GRAPHS';$('#capture-status').textContent=frozenSnapshot?'Ⅱ FROZEN VIEW':'● RECORDING';$('#capture-status').classList.toggle('frozen',!!frozenSnapshot);renderData();};
